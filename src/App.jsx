...
function App() {
  ...
  useEffect(() => {
    // If you refresh, keep whatever the browser/user prefers.
  ...
  useEffect(() => {
    threeRef.current?.setTheme?.(theme);
  }, [theme]);
  ...
  useEffect(() => {
    ...
    function applyTheme(nextTheme) {
      if (nextTheme === 'light') {
        renderer.setClearColor('#f7f9ff', 1);
        scene.background = new THREE.Color('#f7f9ff');
        hemi.color.set('#f8f6ff');
        hemi.groundColor.set('#7e8796');
        dir.intensity = 0.95;
      } else {
        renderer.setClearColor('#000000', 1);
        scene.background = new THREE.Color('#000000');
        hemi.color.set('#f8f6ff');
        hemi.groundColor.set('#080808');
        dir.intensity = 1.1;
      }
    }
    ...
    function createLayerPreviewDataUrl() {
      ...
      if (!ctx) return '';
      // Keep the same background as the active theme for thumbnails.
      ctx.fillStyle = theme === 'light' ? '#f7f9ff' : '#000000';
  ...
  return (
    <div className={`app ${theme === 'light' ? 'theme-light' : 'theme-dark'} ${tool === 'tack' ? 'tack-mode' : ''} ${tool === 'drag' ? 'drag-mode' : ''}`}>
      ...
      <div className={`workspace ${layersCollapsed ? 'layers-collapsed' : ''}`}>
        <div ref={mountRef} className="viewport" />
        <aside className={`layers-pane ${layersCollapsed ? 'collapsed' : ''}`}>
          <div className="layers-pane-header">
            <h3 className="layers-title">Layers</h3>
            <button
              className="layers-minimize-btn"
              onClick={() => setLayersCollapsed((v) => !v)}
              aria-label={layersCollapsed ? 'Expand layers pane' : 'Minimize layers pane'}
              title={layersCollapsed ? 'Expand layers pane' : 'Minimize layers pane'}
            >
              {layersCollapsed ? '➤' : '➤'}
            </button>
          </div>

          <div className="layers-pane-content" aria-hidden={layersCollapsed}>
            <ul className="layers-list">
              {layers.map((layer) => (
                ...
                >
                  <img className="layer-thumb" src={layerPre"
                  />
                </li>
              ))}
            </ul>
